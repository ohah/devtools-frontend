...
let mmkvStoragePanelInstance: MMKVStoragePanel;
let asyncStorageStoragePanelInstance: AsyncStorageStoragePanel;
...
export class StoragePanel extends UI.Panel.PanelWithSidebar implements IMMKVStoragePanel, IAsyncStorageStoragePanel {
...
export class MMKVStoragePanel extends UI.Panel.PanelWithSidebar implements IMMKVStoragePanel {
  visibleView: UI.Widget.Widget|null;
  private pendingViewPromise: Promise<UI.Widget.Widget>|null;
  storageViews: HTMLElement;
  private readonly storageViewToolbar: UI.Toolbar.Toolbar;
  private mmkvStorageView: MMKVStorageItemsView|null;
  private readonly sidebar: MMKVOnlyStoragePanelSidebar;
  ...
  static instance(opts: {forceNew: boolean|null} = {forceNew: null}): MMKVStoragePanel {
    const {forceNew} = opts;
...
export class AsyncStorageStoragePanel extends UI.Panel.PanelWithSidebar implements IAsyncStorageStoragePanel {
  visibleView: UI.Widget.Widget|null;
  private pendingViewPromise: Promise<UI.Widget.Widget>|null;
  storageViews: HTMLElement;
  private readonly storageViewToolbar: UI.Toolbar.Toolbar;
  private asyncStorageStorageView: AsyncStorageStorageItemsView|null;
  private readonly sidebar: AsyncStorageOnlyStoragePanelSidebar;
  ...
  static instance(opts: {forceNew: boolean|null} = {forceNew: null}): AsyncStorageStoragePanel {
    const {forceNew} = opts;
...
class MMKVOnlyStoragePanelSidebar extends UI.Widget.VBox {
  private readonly panel: MMKVStoragePanel;
  private readonly sidebarTree: UI.TreeOutline.TreeOutlineInShadow;
  private mmkvStorageTreeElements: Map<MMKVStorage, MMKVStorageTreeElement>;

  constructor(panel: MMKVStoragePanel) {
    super();
    this.panel = panel;
    this.element.classList.add('storage-panel-sidebar');
    this.sidebarTree = new UI.TreeOutline.TreeOutlineInShadow();
  ...
  private addMMKVStorage(mmkvStorage: MMKVStorage): void {
    ...
    const mmkvStorageTreeElement = new MMKVStorageTreeElement(this.panel as unknown as StoragePanel, mmkvStorage);
    this.mmkvStorageTreeElements.set(mmkvStorage, mmkvStorageTreeElement);
...
class AsyncStorageOnlyStoragePanelSidebar extends UI.Widget.VBox {
  private readonly panel: AsyncStorageStoragePanel;
  private readonly sidebarTree: UI.TreeOutline.TreeOutlineInShadow;
  asyncStorageListTreeElement: ExpandableStoragePanelTreeElement;
  private asyncStorageStorageTreeElements: Map<AsyncStorageStorage, AsyncStorageStorageTreeElement>;

  constructor(panel: AsyncStorageStoragePanel) {
    super();
    this.panel = panel;
    this.element.classList.add('storage-panel-sidebar');
    this.sidebarTree = new UI.TreeOutline.TreeOutlineInShadow();
    ...
    this.asyncStorageListTreeElement = new ExpandableStoragePanelTreeElement(
        this.panel as unknown as StoragePanel,
        'AsyncStorage',
        'No AsyncStorage detected',
        'On this page you can view, add, edit, and delete AsyncStorage key-value pairs.',
        'async-storage');
    const asyncStorageIcon = createIcon('table');
    this.asyncStorageListTreeElement.setLeadingIcons([asyncStorageIcon]);
    this.sidebarTree.appendChild(this.asyncStorageListTreeElement);
  ...
  private addAsyncStorageStorage(asyncStorageStorage: AsyncStorageStorage): void {
    ...
    if (this.asyncStorageStorageTreeElements.size === 0) {
      this.sidebarTree.removeChild(this.asyncStorageListTreeElement);
      const asyncStorageStorageTreeElement =
          new AsyncStorageStorageTreeElement(this.panel as unknown as StoragePanel, asyncStorageStorage);
      this.asyncStorageStorageTreeElements.set(asyncStorageStorage, asyncStorageStorageTreeElement);
      this.sidebarTree.appendChild(asyncStorageStorageTreeElement);
      asyncStorageStorageTreeElement.select();
    } else {
