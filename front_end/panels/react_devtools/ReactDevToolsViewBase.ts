...
export class ReactDevToolsViewBase extends UI.View.SimpleView implements
  ...
  constructor(
    ...
    super({ title, viewId: (`react-devtools-${tab}` as Lowercase<string>) });
  ...
  #renderDevToolsView(): void {
    ...
    if (usingDarkTheme) {
      const applyDarkThemeInputOverrides = (): void => {
        ...
        let styleEl = document.getElementById('react-devtools-dark-input-overrides') as HTMLStyleElement | null;
        if (!styleEl) {
  ...
  #renderErrorView(errorMessage: string): void {
    ...
    const feedbackLink = (globalThis as unknown as { FB_ONLY__reactNativeFeedbackLink?: Platform.DevToolsPath.UrlString })
      .FB_ONLY__reactNativeFeedbackLink;
    if (feedbackLink) {
